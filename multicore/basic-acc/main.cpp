...
public:
	...
	void parse(int argc, char **argv) override {
		...
		assert((mem_end_addr < clint_start_addr || mem_start_addr > sobelFilter0_end_addr) &&
		       "RAM too big, would overlap memory");
		assert((mem_end_addr < clint_start_addr || mem_start_addr > sobelFilter1_end_addr) &&
		       "RAM too big, would overlap memory");
...
int sc_main(int argc, char **argv) {
	...
	DebugMemoryInterface core0_dbg_if("DebugMemoryInterface");
	DebugMemoryInterface core1_dbg_if("DebugMemoryInterface");
	...
	instr_memory_if *core0_instr_mem_if = &iss0_mem_if;
	instr_memory_if *core1_instr_mem_if = &iss1_mem_if;
	data_memory_if *core0_data_mem_if = &iss0_mem_if;
	data_memory_if *core1_data_mem_if = &iss1_mem_if;
	...
	core0.init(core0_instr_mem_if, core0_data_mem_if, &clint, entry_point, rv32_align_address(opt.mem_end_addr));
	core1.init(core1_instr_mem_if, core1_data_mem_if, &clint, entry_point, rv32_align_address(opt.mem_end_addr));
	...
	bus.ports[12] = new PortMapping(opt.sobelFilter0_start_addr, opt.sobelFilter0_end_addr);
	bus.ports[13] = new PortMapping(opt.sobelFilter1_start_addr, opt.sobelFilter1_end_addr);
	...
	bus.isocks[12].bind(sobel_filter0.tsock);
	bus.isocks[13].bind(sobel_filter1.tsock);
