...
struct SimpleDMA : public sc_core::sc_module {
	...
	uint32_t initial_cycles = 2; // 添加initial_cycles成员变量并初始化为2
	...
	void do_transaction(tlm::tlm_command cmd, uint64_t addr, uint8_t *data, unsigned num_bytes) {
		...
		// Calculate the number of cycles for the DMA transaction
    	uint32_t cycles = (trans.get_data_length() / 4) + initial_cycles;  // Assuming 4 bytes per cycle

    	// Add the cycles to the delay
		...
		// Print the number of cycles for the DMA transaction
    	std::cout << "DMA Transaction Cycles: " << cycles << std::endl;
	}
};
